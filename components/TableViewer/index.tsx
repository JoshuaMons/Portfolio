...
interface ColumnRowProps {
  col: ParsedColumn;
  table: ParsedTable;
  onOpen: (col: ParsedColumn) => void;
}

function ColumnRow({ col, table, onOpen }: ColumnRowProps) {
  const { t } = useLanguage();
  return (
    <tr className="hover:bg-surface-50 transition-colors group">
      {/* Column name — click for modal */}
      <td className="px-4 py-3">
        <button
          onClick={() => onOpen(col)}
          className="font-medium text-slate-800 text-sm hover:text-primary-700 hover:underline underline-offset-2 transition-colors text-left"
        >
          {col.originalName}
        </button>
      </td>
      ...
      {/* Inferred type — click for modal */}
      <td className="px-4 py-3">
        <button
          onClick={() => onOpen(col)}
          className={clsx(
            'text-xs font-semibold px-2 py-0.5 rounded-full hover:opacity-80 transition-opacity',
      ...
      {/* Sample values — each pill is clickable */}
        ...
        <div className="flex flex-wrap gap-1">
          {col.sampleValues.slice(0, 3).map((v, i) => (
            <button
              key={i}
              onClick={() => onOpen(col)}
              className="bg-surface-100 hover:bg-primary-50 hover:text-primary-700 border border-transparent hover:border-primary-200 px-1.5 py-0.5 rounded text-slate-600 truncate max-w-[80px] transition-colors"
              title={v}
            >
              {v}
            </button>
          ...
          {col.sampleValues.length > 3 && (
            <button
              onClick={() => onOpen(col)}
              className="text-slate-400 hover:text-primary-600 text-xs transition-colors"
            >
              +{col.sampleValues.length - 3}
            </button>
          )}
...
function TableDetail({ table, onOpenCol }: { table: ParsedTable; onOpenCol: (col: ParsedColumn) => void }) {
  const { t } = useLanguage();
  return (
    <div className="card overflow-hidden">
      <div className="px-5 py-4 border-b border-surface-200 flex items-center justify-between">
        ...
        <span className="text-xs text-slate-400">{t('colModalDescription')} →</span>
      </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-surface-100">
            {table.columns.map((col) => (
              <ColumnRow key={col.originalName} col={col} table={table} onOpen={onOpenCol} />
            ))}
          </tbody>
        </table>
      </div>
...
export default function TableViewer() {
  ...
  const activeTable = tables.find((tbl) => tbl.name === selected) ?? tables[0];
    ...
    <>
      <div className="flex gap-5">
        ...
        <div className="flex-1 min-w-0">
          {activeTable ? (
            <TableDetail table={activeTable} onOpenCol={setModalCol} />
          ) : (
